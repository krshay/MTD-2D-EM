...
if __name__ == '__main__':
    ...
    with plt.style.context('ieee'):
        ...
        plt.loglog(SNRs[:7], errs_EM_mean[:7], '.-b', label=r'Approximate EM')
        # plt.loglog(SNRs, errs_ac_mean[4]*(SNRs/SNRs[4])**(-3/2), 'k--', label='_nolegend_', lw=0.5)

        plt.loglog(SNRs[:7], errs_ac_mean[:7], '.--r', label='Autocorrelation analysis')

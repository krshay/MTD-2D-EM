...
def EM(Ms, c_init, rho_init, L, K, Nd, B, Bk, roots, kvals, nu, sigma2, T):
    z_init = T.H @ c_init
    c_k = c_init
    z_k = z_init
    rho_k = rho_init
    ...
    while True:
        ...
        z_updated = z_step(c_k, pl_phi_k, Ms, B, L, K, Nd, nu, roots, kvals, sigma2, PsiPsi_vals, T)
...
def z_step(c_k, pl_phi_k, Ms, B, L, K, Nd, nu, roots, kvals, sigma2, PsiPsi_vals, T):
    Phi = calc_Phi(K)
    Ls = calc_shifts(L)

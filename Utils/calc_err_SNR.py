...
from Utils.EM_funcs import EM_parallel, rearangeB, PsiPsi, calcB_CTZs, EM
...
#         np.save(f'errs_EM_SNR_{idx}_sd{sd}.npy', errs_EM)
#         np.save(f'errs_ac_SNR_{idx}_sd{sd}.npy', errs_ac)

#         print(f'finished iter #{sd}, SNR = {SNR}. Took {times_EM[idx, 0]} secs. Error for EM = {errs_EM[idx, 0]}; error for autocorrelation analysis = {errs_ac[idx, 0]}.')
...
#         print(f'finished iter #{sd}, SNR = {SNR}. Took {times_EM[idx, 0]} secs. Error for EM = {errs_EM[idx, 0]}; error for autocorrelation analysis = {errs_ac[idx, 0]}; error for deconvolution = {errs_conv[idx, 0]}.')
#     print(f'iter #{sd} finished')
#     return errs_EM, costs_EM, times_EM, errs_ac, costs_ac, times_ac, errs_conv

def calc_err_SNR_startac(L, ne, SNRs, N, gamma, K, sd):
    """ Calculate estimation error in estimating a specific target image, multiple micrograph SNRs. For the case of EM PSF and TSF.
    ...
    for (idx, SNR) in enumerate(SNRs):
        ...
        z_est_parallel, rho_est, log_likelihood, numiters = EM_parallel(Ms, z_est_ac, rho_init, L, K, Nd, B, Bk, kvals, nu, sigma2, BCTZs, PsiPsi_vals, z)
        est_err_coeffs_EM = min_err_coeffs(z, z_est_parallel, kvals)
        errs_EM[idx] = est_err_coeffs_EM[0]
        print(f'SNR = {SNR}. error of {errs_ac[idx]} for aca, eror of {errs_EM[idx]} for EM')

...
The separation condition~(\ref{eq:sep}) implies that each patch $M_m$ can contain either no target image, a full rotated target image, or part of a rotated image; overall there are~$4L^2-1$ possible configurations.  In particular, each patch can be modeled by
\begin{equation}
\label{eq:patch}
...
\KwIn{measurement~$M$; noise variance~$\sigma^2$; initial guesses~$\alpha_0$ and~$\rho_0$; parameter~$K$; stopping parameter~$\epsilon$}
...
calculate~$Q_{0}$ according to~(\ref{eq:Q_func_well_general});\\
  \While{$Q_k - Q_{k-1} > \epsilon$}{
    calculate~$p(M_m|\vec{\ell}, \phi, \alpha_k)$ according to~(\ref{eq:pM_l_phi_alpha});\\
